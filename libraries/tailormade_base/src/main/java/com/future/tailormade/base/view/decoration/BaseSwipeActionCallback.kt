...
abstract class BaseSwipeActionCallback(private val backgroundColor: Int,
  ...
  override fun onChildDraw(canvas: Canvas, recyclerView: RecyclerView,
    ...
    if (isCancelled(dX, isCurrentlyActive)) {
      clearCanvas(canvas, itemView.right.plus(dX), itemView.top.toFloat(), itemView.right.toFloat(),
    ...
    _background.apply {
      ...
      setBounds(itemView.right.plus(dX).toInt(), itemView.top, itemView.right, itemView.bottom)
      draw(canvas)
    }
